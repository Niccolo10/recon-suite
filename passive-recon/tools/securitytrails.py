...
class SecurityTrailsTool(BaseTool):
    ...
    def run(self, domains: List[str]) -> List[Tuple[str, Dict]]:
        """
        Run SecurityTrails enumeration on domains
        Uses multiprocessing with cookie rotation
        """
    ...
    def _process_domain(self, domain_or_ip: str, process_input: Dict, log_lock, results_lock, file_lock, all_results):
        ...
        if not cookie_str:
            self._safe_log(f"[ERROR] No cookie for {domain_or_ip}", log_lock)
            return
        ...
        while page <= total_pages:
            try:
                ...
                if response.status_code != 200:
                    self._safe_log(
                        colored(f"[ERROR] {domain_or_ip} page {page}: status {response.status_code}", "red"),
                        log_lock
                    )
                    break
                ...
                # Different structure for IP vs domain
                if is_ip:
                ...
                self._safe_log(
                    colored(f"[EXC] {domain_or_ip} page {page}: {str(e)}", "magenta"),
                    log_lock
                )
                break
